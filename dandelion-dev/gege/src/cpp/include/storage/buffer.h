...
class PartitionBuffer {
   ...
   public:
    ...
    torch::Tensor getCurrentBufferState() { return buffer_state_; }

    torch::Tensor getNextBufferState() {
        if (buffer_state_iterator_ == buffer_states_.end()) {
            return torch::Tensor();
        }
        return *buffer_state_iterator_;
    }

    int64_t getSlotBytes() { return partition_size_ * embedding_size_ * dtype_size_; }

    int64_t getPartitionBytes(int partition_id) { return partition_table_[partition_id]->partition_size_ * embedding_size_ * dtype_size_; }
...
class MemPartitionBuffer : public PartitionBuffer {
   ...
   public:
    ...
    void finalizeExternalSwap(torch::Tensor next_buffer_state, bool advance_state);
