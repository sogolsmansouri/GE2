...
shared_ptr<StorageBackendConfig> initStorageBackendConfig(pyobj python_config) {
    ...
    if (ret_config->type == StorageBackend::PARTITION_BUFFER || ret_config->type == StorageBackend::MEM_PARTITION_BUFFER) {
        ...
        buffer_options->randomize_partition_ids = cast_helper<bool>(py_options.attr("randomize_partition_ids"));
        buffer_options->shuffle_buffer_states = cast_helper<bool>(py_options.attr("shuffle_buffer_states"));
        buffer_options->inter_gpu_swap = cast_helper<bool>(py_options.attr("inter_gpu_swap"));
        buffer_options->mock_num_devices_for_schedule = cast_helper<int>(py_options.attr("mock_num_devices_for_schedule"));
